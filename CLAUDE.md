...
- **`FALLBACK_COORDS` in `MapView.jsx` filters bad geocodes** — coordinates shared by 8+ different businesses are city-level fallbacks and are excluded from the map. Threshold is 8: University Place Mall has 7 businesses geocoded to the same entrance point (legit cluster); true city-level fallbacks have 9–13 businesses at one point (Provo=13, Saratoga Springs=10, Orem=9).
