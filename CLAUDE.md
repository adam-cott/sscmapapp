...
- **Keep `locationRestriction` wording standardized** — full city names (never "EM", "SF", "PG", "WJ", "Mtn"), "&" between places (comma-separated for 3+: "Orem, Vineyard, Eagle Mountain & Santaquin"), "All ..." for broad scopes ("All Locations", "All Utah County"), and "excluding" for exclusions. The matcher parses this text, so odd wording silently changes which pins show. `deal.description` starts with a copy of it ("Valid at: <restriction> • extras") — update both together.
